...
export function renderDemo(mount: HTMLElement, id: string): () => void {
      ...
      <section class="demo-panel" id="demo-panel" data-expanded="${expanded}">
        ...
        <div class="demo-panel-body" id="demo-panel-body">
          <div class="demo-panel-inner">
            ...
            <div class="demo-tags" id="demo-tags" hidden></div>
            <section class="demo-animations" id="demo-profile" hidden aria-labelledby="demo-profile-title">
              <div class="demo-animations-head">
                <span class="parts-title" id="demo-profile-title">Character</span>
              </div>
              <dl class="demo-profile-list" id="demo-profile-list"></dl>
            </section>
  ...
  /**
   * The subject sheet: what this character is, and how far each statement can be trusted.
   *
   * Every entry carries a source — measured off the reconstruction, observed in the reference, or
   * generated by the tool that produced the mesh — because on a single-photo reconstruction the
   * difference matters more than the statement does. A back that was invented should not read the
   * same as a bone count that was counted.
   */
  type CharacterProfile = {
    tags: ReadonlyArray<{ label: string; kind: string; note?: string }>;
    details: ReadonlyArray<{ label: string; value: string; source: string }>;
  };
  const profile = (model.userData.sculptRuntime as { profile?: CharacterProfile } | undefined)?.profile;
  if (profile && !capture) {
    const tagBox = mount.querySelector<HTMLElement>('#demo-tags');
    if (tagBox && profile.tags.length) {
      tagBox.hidden = false;
      for (const tag of profile.tags) {
        const chip = document.createElement('span');
        chip.className = `demo-tag demo-tag-${tag.kind}`;
        chip.textContent = tag.label;
        if (tag.note) chip.title = tag.note;
        tagBox.appendChild(chip);
      }
    }
    const list = mount.querySelector<HTMLElement>('#demo-profile-list');
    const section = mount.querySelector<HTMLElement>('#demo-profile');
    if (list && section && profile.details.length) {
      section.hidden = false;
      for (const entry of profile.details) {
        const term = document.createElement('dt');
        term.textContent = entry.label;
        const badge = document.createElement('em');
        badge.className = `demo-source demo-source-${entry.source}`;
