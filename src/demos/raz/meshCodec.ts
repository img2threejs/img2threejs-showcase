...
export interface RiggedModel {
  ...
  play(clip: string | number, fadeSeconds?: number, timeScale?: number, startTime?: number): boolean;
...
export function buildRiggedModel(
  ...
  const play = (which: string | number, fadeSeconds = 0.25, timeScale = 1, startTime = 0): boolean => {
    ...
    next.time = THREE.MathUtils.euclideanModulo(startTime, clip.duration);
