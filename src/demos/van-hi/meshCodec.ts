...
export interface DecodedPart {
  ...
  colour: Float32Array;
  /**
   * The same colours before conversion, one byte per channel, exactly as sampled from the source's
   * albedo. Kept because `garmentSeparation` reads hue thresholds off them: the fabric test is
   * stated in sRGB, which is the space the numbers were measured and reasoned about in, and
   * re-deriving it from the linear values would round twice for no gain.
   */
  srgb: Uint8Array;
  index: Uint32Array;
}
...
export function* decodeParts(model: EncodedModel, base64: string): Generator<DecodedPart, void, void> {
  ...
  const decodeOne = (meta: EncodedPart): DecodedPart => {
    ...
    return { meta, position, normal, colour, srgb, index };
  };
