...
const authored: DemoEntry[] = [
  {
    ...
    blurb:
      ...
      + 'at t=2.505 s \u2014 and the orb flies 0.62 figure heights before it bursts. Nothing is fetched: '
      + 'geometry, per-vertex colour and every keyframe are TypeScript.',
    referenceImage: `${BASE}references/chun-li.jpg`,
    // Renders the panel's "Generated by Hyper3D" button, the same field girl-character-3 uses for
    // its Rodin workspace. The URL literal lives here because `registry.ts` is the one file the
    // showcase-safety scan exempts from its no-external-URL rule for demo sources.
    referenceUrl: 'https://hyper3d.ai/workspace/rodin/cc5511d3-a6bb-455f-97e4-a3d1f9967234',
