...
export function createMonsterTreeLights(figureHeight = 1.9): THREE.Group {
  ...
  const fill = new THREE.DirectionalLight(new THREE.Color(PALETTE.studioAmbient).convertSRGBToLinear(), 3.4);
  fill.name = 'fill';
  ...
  const rim = new THREE.DirectionalLight(lifeColour(0.5, 0.95), 1.7);
  rim.name = 'rim';
  rim.position.set(-h * 0.55, h * 0.42, -h * 1.25);
  group.add(rim, rim.target);

  const bounce = new THREE.DirectionalLight(new THREE.Color(PALETTE.barkMid).convertSRGBToLinear(), 2.2);
  bounce.name = 'bounce';
  ...
  const sky = new THREE.HemisphereLight(new THREE.Color(PALETTE.studioAmbient).convertSRGBToLinear(), new THREE.Color(PALETTE.barkMid).convertSRGBToLinear(), 2.2);
  sky.name = 'hemi';
  group.add(sky);
