...
/** Ice-specific skull/neck/collar field. A tall shoulder branch is not a head simply
 * because it reaches crown height. Weld only the solve graph; leave all render buffers intact. */
export function iceNeckField(position:Float32Array,index:Uint32Array):Float32Array {
 ...
 for(const i of points.values()){
  const x=position[i*3],y=position[i*3+1],az=Math.abs(position[i*3+2]);
  if(y<.695||az>.17||(x<-.073&&y<.89))field[i]=0;
  else if((y>.90&&az<.15)||(y>.78&&az<.045&&x>-.03))field[i]=1;
  else {field[i]=THREE.MathUtils.smoothstep(y,.70,.82);if(offsets[i+1]>offsets[i])free.push(i);}
 }
