...
export function decodeUint16s(text: string): Uint16Array {
  const bytes = decodeBase64(text);
...
/**
 * Build the skinned model.
 *
 * SUPERSEDED for this demo by `buildMonsterTreeRig` in `rig.ts`, which fixes three defects
 * measured in this function and documented in `tools/measure-rig.mjs`:
 *
 *   1. `mesh.scale` is set AFTER `mesh.bind()`, so the bind matrix is captured at scale 1 while
 *      the bone world matrices carry the 1.9899x normalise scale. The scale is then applied twice
 *      — once through the skinning matrices and once through `modelViewMatrix` — and the figure
 *      renders at 3.96x with the skin dragging away from the skeleton.
 *   2. `bind(skeleton)` with no explicit bind matrix makes three call `skeleton.calculateInverses()`,
 *      which overwrites the GLB's authored `inverseBind` matrices that `buildSkeleton` just
 *      passed in.
 *   3. `userData.update` reads a `delta` argument the static contract never passes, so the
 *      README's `updateMonster1(model, clock.getElapsedTime())` advances the mixer by 0 forever.
 *
 * Kept here unchanged so the export stays diffable against what the playground emitted.
 */
