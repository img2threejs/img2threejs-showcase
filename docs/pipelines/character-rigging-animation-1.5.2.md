...
**Status:** proposed. **Scope:** computing and designing animation for character rigs.
...
1. **Deform vs technical.** A node is a `Bone` **iff it appears in `skin.joints`**; everything else
   is a `Group`. Do not infer from names. Lee Sin's rig has 42 deform joints inside a much larger
   node graph, and clips target technical nodes that are not joints — building those as bones
   corrupts the skeleton's index space.
